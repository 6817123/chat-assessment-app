...
const FILE_RESPONSES = {
  en: {
    image: "I can see the image you've shared! It looks interesting. What would you like to know about it?",
    audio: "I received your audio message. Unfortunately, I can't process audio files yet, but I appreciate you sharing it with me!",
    document: "Thanks for sharing this document. I can see the file, though I can't read its contents directly. What would you like to discuss about it?"
  },
  ar: {
    image: "أستطيع رؤية الصورة التي شاركتها! تبدو مثيرة للاهتمام. ماذا تريد أن تعرف عنها؟",
    audio: "استلمت رسالتك الصوتية. للأسف، لا يمكنني معالجة الملفات الصوتية حتى الآن، لكنني أقدر مشاركتها معي!",
    document: "شكراً لك على مشاركة هذا المستند. أستطيع رؤية الملف، رغم أنني لا أستطيع قراءة محتواه مباشرة. ماذا تريد أن نناقش حوله؟"
  }
}
...
export async function generateAssistantResponse(
  ...
  // Handle messages with attachments
    ...
    const attachmentType = userMessage.attachments[0].type
    content = FILE_RESPONSES[language][attachmentType]
    
    // Add comment about the text if there's also text content
    if (userMessage.content.trim()) {
      const textComment = language === 'en' 
        ? ` Also, regarding your message: "${userMessage.content}" - I find that quite interesting!`
        : ` أيضاً، بخصوص رسالتك: "${userMessage.content}" - أجد ذلك مثيراً للاهتمام!`
      content += textComment
    }
  }
  // Handle text-only messages
    ...
    if (Math.random() > 0.7 && userMessage.content.length > 10) {
      const echoPrefix = language === 'en' 
        ? ' You mentioned: "' 
        : ' لقد ذكرت: "'
      const echoSuffix = language === 'en'
        ? '". That\'s worth considering.'
        : '". هذا يستحق التفكير.'
      content += echoPrefix + userMessage.content + echoSuffix
    }
  }
...
export function generateConversationTitle(firstMessage: string, language: 'en' | 'ar' = 'en'): string {
  ...
  const commonStarts = language === 'en' 
    ? ['hello', 'hi', 'hey', 'can you', 'could you', 'please', 'i want', 'i need']
    : ['مرحبا', 'أهلا', 'مرحباً', 'أهلاً', 'هل يمكنك', 'هل يمكن', 'من فضلك', 'أريد', 'أحتاج']
...
export async function simulateError(errorType: 'network' | 'timeout' | 'server'): Promise<never> {
  const errorMessages = {
    network: {
